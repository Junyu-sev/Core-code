...
2. **提取进行GWAS分析的个体**
    通过代码自动生成并提交LSF任务。提取目标个体，将每个染色体bgen原始数据转换为plink2的pgen格式。这是在师姐代码基础上改动的，还没有得到师姐授权（小声）。
...
3. **合并基因型数据**
    将提取得到的1-22号染色体数据合并，生成plink格式的.bed文件。这个也是在师姐代码基础上改动的，还没有得到师姐授权（小声）。
...
4. **进行GWAS质控**
    ...
    /share/home/zhangjunyu/anaconda3/envs/PRS/bin/Rscript - <<EOF
    library(data.table)
    library(tidyverse)
