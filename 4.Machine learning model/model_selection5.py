# 采用过采样方法，看一下跑出来的评估指标有没有变好很多
# 在k-fold得到的训练集中进行过采样，测试集不变
# 结论来说，评估指标没有变好（持平，甚至有些更差了）
# 但是如果也同时过采样测试集，结果变好了很多
# 本质上是模型的recall还行，所以提高阳性样本的数目时，模型拟合效果变好了
# 代码内容参考model_selections3.py和model_selections4.py
...
def evaluate_model(model):
    ...
    for i in range(10):
        ...
        smote = SMOTE()
        #X_train, Y_train = smote.fit_resample(X_train, Y_train)#训练集过采样
        X_test =  X.loc[X['Region_code'] == i].drop(['Region_code'], axis=1)
        Y_test = Y.loc[Y['Region_code'] == i].drop(['Region_code'], axis=1)
        X_test, Y_test = smote.fit_resample(X_test, Y_test)#测试机过采样
